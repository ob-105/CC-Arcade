...
local function writeStartup(programPath)
    ...
    file.writeLine("local REPO = \"https://raw.githubusercontent.com/ob-105/CC-Arcade/main/\"")
    file.writeLine("local roleProgram = \"" .. programPath .. "\"")
    file.writeLine("local roleFiles = {")
    file.writeLine("  server = { \"shared/protocol.lua\", \"shared/security.lua\", \"shared/net.lua\", \"shared/updater.lua\", \"server/main.lua\" },")
    ...
    file.writeLine("  if not fs.exists(\"/arcade_role.txt\") then return nil end")
    file.writeLine("  local f = fs.open(\"/arcade_role.txt\", \"r\")")
    file.writeLine("  if not f then return nil end")
    file.writeLine("  local v = f.readAll() or \"\"")
    file.writeLine("  f.close()")
    file.writeLine("  v = string.gsub(v, \"^%s+\", \"\")")
    file.writeLine("  v = string.gsub(v, \"%s+$\", \"\")")
    file.writeLine("  if v == \"\" then return nil end")
    file.writeLine("  return v")
    file.writeLine("end")
    file.writeLine("")
    file.writeLine("local function ensureParent(path)")
    file.writeLine("  local d = fs.getDir(path)")
    file.writeLine("  if d and d ~= \"\" and not fs.exists(d) then fs.makeDir(d) end")
    file.writeLine("end")
    file.writeLine("")
    file.writeLine("local function download(relative)")
    file.writeLine("  if not http then return false, \"HTTP disabled\" end")
    file.writeLine("  local res, err = http.get(REPO .. relative)")
    file.writeLine("  if not res then return false, tostring(err) end")
    file.writeLine("  local body = res.readAll()")
    file.writeLine("  res.close()")
    file.writeLine("  if not body or body == \"\" then return false, \"empty response\" end")
    file.writeLine("  local out = \"/\" .. relative")
    file.writeLine("  ensureParent(out)")
    file.writeLine("  local f = fs.open(out, \"w\")")
    file.writeLine("  if not f then return false, \"write failed\" end")
    file.writeLine("  f.write(body)")
    file.writeLine("  f.close()")
    file.writeLine("  return true, nil")
    file.writeLine("end")
    file.writeLine("")
    file.writeLine("local role = readRole()")
    file.writeLine("if role and roleFiles[role] then")
    file.writeLine("  for _, rel in ipairs(roleFiles[role]) do")
    file.writeLine("    local target = \"/\" .. rel")
    file.writeLine("    if not fs.exists(target) then")
    file.writeLine("      local ok, err = download(rel)")
    file.writeLine("      if not ok then")
    file.writeLine("        print(\"[startup] bootstrap failed for \" .. rel .. \" (\" .. tostring(err) .. \")\")")
    file.writeLine("      end")
    file.writeLine("    end")
    file.writeLine("  end")
    file.writeLine("end")
    ...
    file.writeLine("  local updateOk, updateErr = pcall(updater.runUpdate)")
    file.writeLine("  if not updateOk then")
    file.writeLine("    print(\"[startup] updater runtime error: \" .. tostring(updateErr))")
    ...
    file.writeLine("end")
    file.writeLine("")
    file.writeLine("if not fs.exists(roleProgram) then")
    file.writeLine("  print(\"[startup] missing program \" .. roleProgram)")
    file.writeLine("  if role and roleFiles[role] then")
    file.writeLine("    local rel = string.sub(roleProgram, 2)")
    file.writeLine("    local got, err = download(rel)")
    file.writeLine("    if got then")
    file.writeLine("      print(\"[startup] downloaded \" .. rel)")
    file.writeLine("    else")
    file.writeLine("      print(\"[startup] final download failed: \" .. tostring(err))")
    file.writeLine("    end")
    file.writeLine("  end")
    file.writeLine("end")
    file.writeLine("")
    file.writeLine("if fs.exists(roleProgram) then")
    file.writeLine("  shell.run(roleProgram)")
    file.writeLine("else")
    file.writeLine("  print(\"No such program: \" .. roleProgram)")
    file.writeLine("  print(\"Run /install.lua again once HTTP is enabled.\")")
    file.writeLine("end")
    file.close()
